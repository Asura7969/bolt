...
import org.bolt.rpc.{RpcAddress, RpcEndpoint, RpcEndpointAddress, RpcEndpointNotFoundException, RpcEndpointRef, RpcEnv, RpcEnvConfig, RpcEnvFactory, RpcEnvStoppedException, RpcTimeout}
import org.bolt.serializer.{JavaSerializer, JavaSerializerInstance}
import org.bolt.util.{ThreadUtils, Utils}
import org.apache.spark.network.{TransportContext, client}
import org.apache.spark.network.client.{RpcResponseCallback, _}
...
class NettyRpcEnv(
  ...
  private[netty] val transportConf: TransportConf = BoltTransportConf.fromSparkConf(
